...
# ---- Dateien-Auswahl mit Speicher für zuletzt genutztes Verzeichnis ----
...
# ---- Kennzahlen Berechnung (Basic + erweitertes Risiko-Management) ----
...
def pyramiding_strategy(position, current_price, atr):
    ...
    unrealized_pnl = (current_price - position['entry_price']) * position['size_coins']
    if unrealized_pnl > 2 * atr and position['pyramid_levels'] < 2:
        add_size_coins = position['size_coins'] * 0.5
        position['pyramid_levels'] += 1
        return add_size_coins
    return 0


# ---- Simulationsfunktion (vollständig) ----
def simulate_advanced_strategy(
    ...
    trade_counter = 1  # Für Entry/Exit-Marker-Chart
        ...
        for pos in positions[:]:
            ...
            if exit_reason:
                ...
                trades.append({
                    ...
                    'risk_reward_ratio': pos['max_gain'] / pos['max_loss'] if pos['max_loss'] > 0 else np.nan,
                    'regime': pos['regime'],
...
def calculate_enhanced_metrics(trades_df, equity_df, initial_capital):
    ...
    if not trades_df.empty:
        metrics.update({
            ...
            'profit_factor': abs(trades_df[trades_df['profit'] > 0]['profit'].sum() / trades_df[trades_df['profit'] < 0]['profit'].sum()),
...
# ---- Verschiedene erweiterte Auswertungen ----
...
# ---- Export Funktion inkl. Excel mit mehreren Sheets und HTML Dashboard ----
def export_comprehensive_results(global_trades, global_equity, all_metrics, timestamp):
    ...
    # HTML Dashboards erzeugen
    html_file_dashboard = create_advanced_dashboard(global_trades, global_equity, all_metrics, timestamp)
    html_file_marker_chart = create_trade_marker_chart(global_trades, global_equity, timestamp)
    return {'csv': csv_file, 'excel': excel_file, 'html': html_file_dashboard, 'html_trades': html_file_marker_chart}


# ---- Erweiterter Portfolio-Dashboard Chart (mit Subplots) ----
def create_advanced_dashboard(trades_df, equity_df, all_metrics, timestamp):
    if trades_df.empty:
        return None
    equity_df['timestamp'] = pd.to_datetime(equity_df['timestamp'])
    # Portfolio Equity je timestamp summiert
    ...
    # 12. Monte Carlo (falls vorhanden)
...
# ---- Neuer Chart mit Marker für Entry/Exit: Dreiecke, Linien, hoverinfo ----
def create_trade_marker_chart(trades_df, equity_df, timestamp):
    ...
    # Trade-Marker & Verbindungslinien
    for _, tr in trades_df.iterrows():
        color = "green" if tr["profit"] >= 0 else "red"

        # Linie zwischen Entry und Exit Kapital
        fig.add_trace(go.Scatter(
...
# ---- Hauptfunktion, die alles zusammenführt und ausgibt ----
def main_complete():
    logging.basicConfig(
    ...
    for file_path in tqdm(file_paths, desc="Verarbeite Assets"):
        try:
            ...
            if not trades_df.empty:
                ...
                metrics['symbol'] = symbol  # für Übersicht
                all_metrics.append(metrics)

            print(f"✅ {symbol}: {len(trades_df)} Trades, Start: 10.000€ → Ende: {final_capital:,.2f}€ "
