# analysis/tradingmodule_250806d.py
...
SKRIPT_ID = "tradingmodule_250806d"
RESULTS_DIR = "results"
...
if os.path.exists(PARAMS_PATH):
    try:
        ...
        print(f"[WARN] Fehler beim Laden von best_params.json ({e}), verwende Standardparameter.")
        global_params = {}
else:
    print("[WARN] Keine best_params.json gefunden, verwende Standardparameter.")
    global_params = {}
...
def run_backtest_for_file(parquet_path, trade_every=1, params=None):
    ...
    logger.info(f"Verarbeite: {asset} ({freq}) mit optimierten Parametern")
...
def export_params_to_excel(writer, params: dict):
    df_params = pd.DataFrame(params.items(), columns=["Parameter", "Wert"])
    df_params.to_excel(writer, sheet_name="Used_Params", index=False)


def export_params_to_markdown(params: dict, path="results/used_params.md"):
    lines = ["# Verwendete Parameter (aus best_params.json)", ""]
...
def main():
    ...
    with pd.ExcelWriter(excel_path, engine="xlsxwriter") as writer:
        ...
        export_params_to_excel(writer, global_params)
