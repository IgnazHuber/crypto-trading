...
def plot_candlestick_trades(df, trades, filename):
    ...
    for _, t in trades.iterrows():
        plt.scatter(t['Entry Time'], t['Entry Price'], color="green" if t['PnL_abs'] >= 0 else "red", s=40, marker="^", label="Entry" if _==0 else "")
        plt.scatter(t['Exit Time'], t['Exit Price'], color="green" if t['PnL_abs'] >= 0 else "red", s=40, marker="v", label="Exit" if _==0 else "")
...
def plot_candlestick_trades_plotly(df, trades, strategy_name, filename):
    if trades.empty:
        print(f"[plotly] Keine Trades für {strategy_name} – Plot übersprungen.")
        return
    fig = go.Figure()
    ...
    for _, t in trades.iterrows():
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=(
                ...
                f"Entry-Preis: {t['Entry Price']:.2f}<br>"
                f"Einsatz: {t['Einsatz']:.2f}<br>"
            ),
            showlegend=False
        ))
        # Exit Marker
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=(
                ...
                f"Exit-Preis: {t['Exit Price']:.2f}<br>"
                f"Gewinn/Verlust: {t['PnL_abs']:.2f} €<br>"
                f"Gewinn/Verlust: {t['PnL_pct']:.2f}%<br>"
                f"Kapital nach Trade: {t['Kapital nach Trade']:.2f} €<br>"
            ),
            showlegend=False
        ))
        # Linie Entry -> Exit
    ...
    fig.update_layout(
        title=f"{ASSET} – {strategy_name}: Trades",
        xaxis_title="Zeit",
        yaxis_title="Preis",
        hovermode='closest',
    ...
    print(f"[plotly] Chart gespeichert: {filename}")
...
def main():
    ...
    for i, (strat_name, candle_key, regime_key, regime_desc) in enumerate(tqdm(STRATEGY_CONFIG, desc="Strategien", ncols=80)):
        ...
        # Plotly-Chart für diese Strategie
        html_chart_path = os.path.join(RESULTS_DIR, f"{safe_sheet_name(strat_name)}.html")
        plot_candlestick_trades_plotly(df, trades, strat_name, html_chart_path)
        # KPI-Berechnung
        if not trades.empty:
