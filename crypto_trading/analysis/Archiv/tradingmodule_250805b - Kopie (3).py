...
def export_summary(summary_df, all_trades_df, equity_dict):
    ...
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
        ...
        # Balkendiagramm Endkapital (grün/rot getrennt)
        ...
        # Portfolio Tab
        portfolio_df = build_portfolio_equity(equity_dict)
        portfolio_tab = "Portfolio_Equity"
        portfolio_df.to_excel(writer, sheet_name=portfolio_tab, index=False)
        rows = len(portfolio_df)
        if rows > 1:
            chart_portfolio = workbook.add_chart({'type': 'line'})
            chart_portfolio.add_series({
                'name': 'Portfolio',
                'categories': [portfolio_tab, 1, 0, rows, 0],
                'values':     [portfolio_tab, 1, 1, rows, 1]
            })
            chart_portfolio.set_title({'name': "Portfolio Equity"})
            writer.sheets[portfolio_tab].insert_chart("E2", chart_portfolio)
