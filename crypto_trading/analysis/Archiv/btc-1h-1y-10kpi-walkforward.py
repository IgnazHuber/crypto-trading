...
def run_strategy(df, candle_func, regime_mask, strat_name, regime_name,
                 start_capital=START_CAPITAL, asset=ASSET, parquet_file=None):
    trades = []
    ...
    for idx in df.index:
        if not in_position:
            if candle_func(df).loc[idx] and regime_mask.loc[idx] and capital > 0:
                in_position = True
                entry_price = df.loc[idx, 'close']
                entry_time = idx  # Index (Position) statt Timestamp
                entry_capital = min(capital * 0.20, capital)  # max 20% Einsatz
        else:
            price_now = df.loc[idx, 'close']
            max_loss_price = entry_price * 0.95
            max_gain_price = entry_price * 1.20
            max_duration = 30  # max 30 Kerzen
            pos_idx = df.index.get_loc(idx)
            pos_entry = df.index.get_loc(entry_time)
            exit_condition = (price_now <= max_loss_price) or (price_now >= max_gain_price) or (pos_idx - pos_entry > max_duration)
            if exit_condition:
                exit_price = price_now
                exit_time = idx  # Index statt Timestamp
                pnl_abs = (exit_price - entry_price) * (entry_capital / entry_price)
                pnl_pct = pnl_abs / entry_capital * 100
                capital += pnl_abs
                trades.append({
                    "trade_id": trade_id,
                    "asset": asset,
                    ...
                    "Strategy": strat_name,
                    "Regime": regime_name,
                    "Entry Time": df.loc[entry_time, 'timestamp'] if 'timestamp' in df else entry_time,
                    "Exit Time": df.loc[exit_time, 'timestamp'] if 'timestamp' in df else exit_time,
                    "Entry Price": entry_price,
                    "Exit Price": exit_price,
                    "Einsatz": entry_capital,
                    "PnL_abs": pnl_abs,
                    "PnL_pct": pnl_pct,
                    "Kapital_nach_Trade": capital
                })
                trade_id += 1
                in_position = False
                if capital <= 0:
                    print("Kapital komplett aufgebraucht, Stopp.")
                    break
...
def plot_interactive_candlestick_with_trades(df, trades, filename_html, filename_png=None, title=None):
    ...
    capital = trades_sorted['Kapital_nach_Trade']
    fig.add_trace(go.Scatter(
...
def simulate_window(params):
    ...
    if not trades_oos.empty:
        ...
        end_capital = trades_oos['Kapital_nach_Trade'].iloc[-1]
    else:
        total_pnl = 0
        win_rate = 0
        num_trades = 0
