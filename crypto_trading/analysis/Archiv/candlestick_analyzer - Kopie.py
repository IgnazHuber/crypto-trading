...
try:
    logging.basicConfig(
        filename='candlestick_analyzer.log',
        level=logging.DEBUG,
...
def detect_candlestick_patterns(df):
    ...
    # Fortschrittsbalken für die Analyse der Zeilen
    for i in tqdm(range(2, len(df)), desc="Analyse Candlestick-Formationen", leave=False):
        prev2 = df.iloc[i-2]
        prev = df.iloc[i-1]
        curr = df.iloc[i]
        
        # Initialisieren der Pattern-Informationen
        pattern = {
        ...
        # Hilfsfunktionen für Kerzen-Eigenschaften
...
def plot_candlestick_chart(df, patterns, output_file='candlestick_chart.html', max_annotations=100):
    ...
    try:
        ...
        # Markierungen für erkannte Formationen (begrenzt auf max_annotations)
...
def process_file(file_path, chunk_size=1000):
    ...
    try:
        ...
        # Prüfen, ob die Datei klein genug ist, um ohne Chunks verarbeitet zu werden
        ...
        if num_rows <= chunk_size:
            ...
            df = df_info
            # Prüfen, ob der Index ein datetime-Format hat
            ...
            # Chunk-Verarbeitung mit Überlappung
            logging.debug(f"Verarbeite Datei in Chunks von {chunk_size} Zeilen")
            overlap = 2  # Überlappung für 3-Kerzen-Muster
            chunk_index = 0
        ...
        # Grafische Ausgabe (letzte 1000 Zeilen)
...
def select_files(last_path):
    ...
    try:
        root = Tk()
        root.withdraw()  # Verberge das Hauptfenster
        ...
        # Speichere den Pfad des Verzeichnisses der letzten Datei
        if file_paths:
...
def main():
    ...
    # Lade den letzten Pfad, falls vorhanden
    last_path = os.getcwd()
    ...
    if all_patterns:
        try:
            ...
            output_csv = f"candlestick_patterns_{timestamp}.csv"
            ...
            output_excel = f"candlestick_patterns_{timestamp}.xlsx"
