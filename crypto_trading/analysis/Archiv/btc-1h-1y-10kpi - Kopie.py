...
def safe_sheet_name(name, maxlen=28):
    """Entfernt ungültige Excel-Zeichen aus Sheetnamen."""
...
def plot_candlestick_trades(df, trades, filename):
    """Zeichnet einen Candlestickchart mit allen Entry/Exit-Positionen, Trade-Nummern."""
    plt.figure(figsize=(18,6))
    plt.plot(df['timestamp'], df['close'], label="Kurs (Close)", color='black', linewidth=1.2)
    # Marker für alle Entry/Exit
...
def main():
    ...
    for strat_name, candle_key, regime_key, regime_desc in tqdm(STRATEGY_CONFIG, desc="Strategien", ncols=80):
        ...
        # KPIs
        if not trades.empty:
            trades['Cum Capital'] = trades['PnL_abs'].cumsum() + START_CAPITAL
    ...
    print("\n=== Zusammenfassung (alle Strategien, nur wenn beides erfüllt ist) ===")
