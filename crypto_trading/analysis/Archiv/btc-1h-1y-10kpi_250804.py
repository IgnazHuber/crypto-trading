...
# === Helper für robustes Spaltennamen-Mapping ===
...
def main():
    ...
    for i, (strat_name, candle_key, regime_key, regime_desc) in enumerate(tqdm(STRATEGY_CONFIG, desc="Strategien", ncols=80)):
        ...
        plot_strategy_chart_with_equity(df, trades, strat_name, html_chart_path)
        if not trades.empty:
    ...
    with pd.ExcelWriter(OUTPUT_EXCEL, engine="openpyxl") as writer:
        ...
        # --- Hier: Automatisch die Legacy-Spalten sicherstellen ---
        all_trades_df = ensure_legacy_trade_columns(all_trades_df)
        ...
        for i, trades in enumerate(all_trade_dfs):
            ...
            t_df = ensure_legacy_trade_columns(trades)
            t_df.to_excel(writer, sheet_name=strat, index=False)
    print(f"\nErgebnisse exportiert nach {OUTPUT_EXCEL}.")

    # Falls gewünscht, Übergabe an Analyse/Reporting-Modul (Beispiel):
    try:
        from crypto_trading.visualization.compare_strategies import compare_strategies_html
        all_trades_df = ensure_legacy_trade_columns(all_trades_df)
        compare_strategies_html(all_trades_df)
    except Exception as ex:
        print("[WARNUNG] compare_strategies_html konnte nicht ausgeführt werden:", ex)
