...
def simulate_advanced_strategy(
    ...
    for i in tqdm(range(len(patterns)), desc="Simuliere Advanced Trades"):
        ...
        if (row['signal'] in ['Kaufen', 'Verkaufen']
            ...
            position_size_eur = calculate_position_size(
                capital, max_position_pct, max_risk_pct, current_data['atr'])
            position_size_eur = min(position_size_eur, capital)  # Kapitalbegrenzung streng

            size_coins = position_size_eur / current_price if current_price > 0 else 0  # 🔧 Coins bestimmen
...
def create_advanced_dashboard(trades_df, equity_df, all_metrics, timestamp):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate=(
                ...
                'Entry: %{customdata[1]}'
                'Exit: %{customdata[2]}'
                'Einsatz: %{customdata[3]:,.2f} €'
                'Profit: %{y:,.2f} €'
                'R/R: %{x}'
                'Exit-Grund: %{customdata[4]}'
                'Regime: %{customdata[5]}'
            ),
            customdata=trades_df[['symbol', 'entry_time', 'exit_time', 'size_eur', 'exit_reason', 'regime']].values  # 🔧 Spalte size_eur!
        ),
        row=3, col=2
    )
    # 9. Exit-Gründe
    ...
    # 12. Monte Carlo (wenn genügend Daten)
