...
from .mod_excel import export_to_excel  # behält Diagramme bei
...
def add_params_to_png_legend(params: dict, output_path):
    """Zeigt die wichtigsten Parameter in einer kleinen Legende im PNG."""
    legend_text = "\n".join([f"{k}: {v}" for k, v in params.items() if k in ["min_signals", "ema_length", "rsi_length", "atr_length", "bb_std"]])
    fig, ax = plt.subplots(figsize=(2, 2))
    ax.text(0.1, 0.5, legend_text, fontsize=8, va="center", ha="left")
    ax.axis("off")
...
def main():
    ...
    for parquet_path in parquet_files:
        ...
        # zusätzliche kleine PNG-Legende mit Parametern
        legend_path = os.path.join(RESULTS_DIR, f"{file_label}_params.png")
        add_params_to_png_legend(global_params, legend_path)
    ...
    # Parameter-Doku separat ablegen
