...
def plot_equity_curve(eq, filename):
...
def export_pdf_report(all_trades, all_perf, asset_param_results, tmpdir, filename="strategy_report.pdf"):
    ...
    for asset, paramsets in asset_param_results.items():
        ...
        for param_idx, summary in paramsets.items():
            ...
            pdf.ln(2)
            pdf.image(monfile, w=100)
            pdf.ln(1)
            # Equity Curve Bild
            pdf.image(eqfile, w=120)
            pdf.ln(1)
            # Kennzahlen-Tabelle
...
asset_param_results = {}  # asset -> {param_idx: {trades, perf, eqfile, monfile}}
