...
# Wichtigkeitsreihenfolge der Indikatoren (50 Stück, keine entfernt)
INDICATOR_PRIORITY = [
...
# Dummy-Simulation ersetzt echte Trades
def simulate_trades_with_capital(df, params):
    capital = 10000.0
    max_loss = 0.0
    # einfache Dummy-PnL-Logik (z. B. 50 USD Gewinn pro 50 Candles)
    for _ in range(len(df) // 50):
        pnl = 50
        capital += pnl
        max_loss = min(max_loss, pnl)
    return capital, max_loss

def optimize_param_set(df_ctx, params):
...
def optimize_for_context(df_ctx, param_grid):
    ...
    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(optimize_param_set, df_ctx, p) for p in param_grid]
        for f in tqdm(as_completed(futures), total=len(futures), desc=f"{df_ctx['market_context'].iloc[0]} Parameter"):
...
def main():
    ...
    # erweiterte Reports
    export_excel_summary(all_results_df, excel_path)
    export_html_summary(all_results_df, os.path.join(RESULTS_DIR, f"optimization_results_by_context_{timestamp}.html"))
    export_markdown_params(best_params_by_context, os.path.join(RESULTS_DIR, "used_params.md"))
